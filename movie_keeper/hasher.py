...
def _cleanliness_score(path: Path) -> Tuple[int, int, int, str]:
    """Lower is "cleaner"; used to choose the canonical file to keep."""
    name = path.stem
    noise_hits = sum(len(p.findall(name)) for p in _NOISE_PATTERNS)
    non_alnum = sum(1 for ch in name if not ch.isalnum() and ch not in (" ", "-", "_", "."))
...
def select_file_to_keep(duplicates_list: Iterable[PathLike]) -> Tuple[Path, List[Path]]:
    ...
    Returns a tuple ``(keep, to_remove)`` where ``to_remove`` is everything
    else in the input. Raises ``ValueError`` for empty input.
    """
